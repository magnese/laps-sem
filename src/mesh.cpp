...
typedef enum{NODE,ELEMENT,GHOST,NONE} flag_type;
typedef enum{FIRST,SECOND,OTHER} flag_temp;
...
void mesh::read_file(string file_name) {
  ...
  if(file.is_open()){
    while(getline(file,my_str)){
      if(my_str.compare("$Nodes")==0) my_flag=NODE;
      if(my_str.compare("$EndNodes")==0){
        my_flag=NONE;
        my_flag2=FIRST;
      }
      if(my_str.compare("$Elements")==0) my_flag=ELEMENT;
      ...
      if(my_flag==NODE){
        ...
        if(my_flag2==FIRST) my_flag2=SECOND;
      }
        ...
        if(my_flag2==OTHER){
          ...
          if (i_temp3==3){
            ...
            for(int i=0;i!=i_temp;++i){
              if(i==2) my_str_converted>>i_temp4;
              else my_str_converted>>i_temp2; // put tags in the garbage
            }
...
void mesh::eval_topology(){
  ...
  for(int i=0;i!=my_elements.num_elements();++i){
    for(int j=*(xadj_ptr+i);j!=*(xadj_ptr+i+1);++j) my_elements.add_n(static_cast<int>(*(adjncy_ptr+j)),i);
  }
  METIS_Free(xadj_ptr);
...
void mesh::reorder_neighbors(void){
  ...
  for(int j=0;j!=my_elements.num_elements();++j){
    ...
    for(int k=0;k!=my_elements.num_neighbors(j);++k){
      ...
      else my_elements.inc_edge_position(map[k]-3,j);
    }
    ...
    for(int i=0;i!=4;++i){
      if(missing_position[i]==0) map.push_back(i);
    }
    ...
    for(int k=0;k!=(my_elements.num_neighbors(j));++k) temp_neigh[map[k]]=my_elements.n(j)[k];
...
int mesh::find_positions_neighbors(const int& j,const int& k){
  ...
  if(temp==1) position[0]+=4;
  else{if((position[1]-position[0])>1) position[0]=position[1];}
  return position[0];
}
...
void mesh::eval_border(void){
  ...
  if(my_rank==0) recv_buffer.resize(static_cast<unsigned>(2*my_lines.num_lines()*my_size),0);
  ...
  if(my_rank==0){
    for(int j=0;j!=(2*my_lines.num_lines());++j){
      for(int jj=1;jj!=my_size;++jj) temp_values[j]+=recv_buffer[j+2*my_lines.num_lines()*jj];
    }
  }
...
void mesh::set_border_function(double (*fptr)(double,double,double),const int& starting_line,const int& ending_line, const flag_bc& type,const int& dof){
  ...
  if(ending_line<starting_line){
    for(int j=(starting_line-my_lines.i(0));j!=(my_lines.num_lines()-my_lines.i(0));++j) my_lines.set_lt(idx,j);
    for(int j=0;j!=(ending_line+1-my_lines.i(0));++j) my_lines.set_lt(idx,j);
  }
  else{
    for(int j=(starting_line-my_lines.i(0));j!=(ending_line+1-my_lines.i(0));++j) my_lines.set_lt(idx,j);
  }
...
void mesh::set_periodicity_mesh(const int& starting_line,const int& ending_line,const int& cor_starting_line,const int& cor_ending_line){
  ...
  else set_border_function(&null_fun,cor_starting_line,cor_ending_line,PERIODICITY);
...
void mesh::write_file(string file_name,const flag_ghost& flag,vector<double>& nodal_values){
  ...
  if (file.is_open()) {
    ...
    file<<"$EndNodes\n$Elements"<<endl;
    if(flag==WRITE_GHOSTS_ON) file<<my_elements.num_elements()+my_elements.num_ghosts()+my_lines.num_lines()<<endl;
    else file<<my_elements.num_elements()+my_lines.num_lines()<<endl;
    for(int j=0;j!=my_lines.num_lines();++j) file<<my_lines.i(j)<<" 1 3 0 "<<my_lines.elem_idx(j)<<" "<<my_lines.lt(j)<<" "<<my_lines.p(j)[0]+1<<" "<<my_lines.p(j)[1]+1<<endl;
    for(int i=0;i!=my_elements.num_elements();++i){
    ...
    if(static_cast<int>(nodal_values.size())==my_nodes.num_nodes()){
      ...
      file<<"1"<<endl;
      file<<my_nodes.num_nodes()<<endl;
      for(int j=0;j!=my_nodes.num_nodes();++j) file<<my_nodes.i(j)<<" "<<nodal_values[j]<<endl;
...
int mesh::corresponding_edge(const int& pos,const int& j){
  ...
  if(my_elements.n(pos,j)>-1){
    for(int i=0;i!=4;++i){
      if(my_elements.v(j)[pos]==my_elements.v(my_elements.n(pos,j))[i]) temp=(i+3)%4;
    }
  }
  return temp;
}
...
void mesh::set_neighbors_cor_pos(void){
  for(int j=0;j!=num_elements();++j){
    for(int jj=0;jj!=4;++jj) my_elements.set_ncp(corresponding_edge(jj,j),jj,j);
  }
}
