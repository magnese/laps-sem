...
public:
  ...
  modes_mapping(M* mesh_ptr,B* basis_ptr, P* p_state_ptr):mesh(mesh_ptr),basis(basis_ptr),p_state(p_state_ptr),
    node_mode_mapping(mesh->num_nodes(),-1),num_dof_nodes(0),num_not_dof_nodes(0),mode_mapping_starting_idx(0),
    elmdist_dof_edges(mesh->get_size()+1,0),num_dof_edges(0),elmdist_not_dof_edges(mesh->get_size()+1,0),num_not_dof_edges(0),
    elmdist_dof_interiors(mesh->get_size()+1,0),num_dof_interiors(0),vtx_map(4),offset_inner_modes(0),dim_boundary_modes(0),sol_coeff(0),
    coeff(0),val(0){
      vtx_map[0]=3;
      vtx_map[1]=0;
      vtx_map[2]=2;
  ...
  inline bool is_v(const int& i,const int& ii){if((i<2)&&(ii<2)) return true; else return false;}
  inline bool is_e(const int& i,const int& ii){if(((i<2)&&(ii>1))||((i>1)&&(ii<2))) return true; else return false;}
  inline bool is_i(const int& i,const int& ii){if((i>1)&&(ii>1)) return true; else return false;}
...
void modes_mapping<M,B,P>::create_mapping(bool static_condensation_status){
  ...
  for(int j=0;j!=mesh->num_elements();++j){
    ...
    for(int jj=0;jj!=4;++jj){
      if(mesh->n(jj,j)>-1){
        if(mesh->i(j)<mesh->i(mesh->n(jj,j))) num_dof_edges+=mesh->get_edge_deg(jj,j)-1;
      }
      else{
        if(mesh->get_bc_type(-mesh->n(jj,j)-1)==DIRICHLET) num_not_dof_edges+=mesh->get_deg(j)-1;
        else num_dof_edges+=mesh->get_deg(j)-1;
      }
    }
  }
  ...
  if(static_condensation_status) offset_inner_modes=dim_boundary_modes;
...
void modes_mapping<M,B,P>::disp_mapping(void){
  ...
  for(int i=0;i!=mesh->get_size();++i){
    ...
    if(mesh->get_rank()==i){
      cout<<endl<<"Edge modes mapping on process "<<i;
...
double& modes_mapping<M,B,P>::u(const int& i,const int& ii,const int& e){
  ...
  if(is_v(i,ii)) coeff=(*sol_coeff)[vtx(i,ii,e)];
  else{
    if(is_i(i,ii)){
      if((i>mesh->get_deg(e))||(ii>mesh->get_deg(e))) coeff=0;
      else coeff=(*sol_coeff)[inner(i,ii,e)];
    }
    else{
      if(max(i,ii)>mesh->get_edge_deg(edge_pos(i,ii),e)) coeff =0;
      else{
