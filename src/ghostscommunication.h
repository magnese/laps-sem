...
public:
  ...
  ghosts_communication(M* mesh_ptr):my_mesh(mesh_ptr),data_size(0),global_vec(),local_vec(),ghosts_idx(0),local_ghosts_positions(0),ghosts_values(0){
    VecCreate(MPI_COMM_WORLD,&local_vec);
  }

  inline void set_size(const int& new_size){data_size=new_size;set_idxs();
  ...
  inline void free(void){PetscFree(global_vec);VecDestroy(&local_vec);}
...
vector<double>& ghosts_communication<M>::get_ghosts_global_idx(void){
    ...
    for (vector<double>::iterator iter=values.begin();iter!=values.end();++iter) *iter=static_cast<double>(my_mesh->get_elements().ele(iter-values.begin()).i());
