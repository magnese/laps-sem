...
public:
  ...
  element():global_index(0),ghost_flag(0),vertex_indices(4,0),neighboring_elements(0),corner_position(4,4),surface_tag(0),neighbor_cor_pos(4,-1){};
  ...
  inline void free_pos(void){for(int j=0; j!=4;++j) corner_position[j]=4;}
  ...
  inline void inc_edge_position (const int& l){for(int j=l;j!=4;++j) corner_position[j]+=1;}
