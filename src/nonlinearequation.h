...
public:
  ...
  non_linear_equation(M* mesh_ptr,double toll,double (*fptr)(double,double)):equation<M,B,G>(mesh_ptr),initial_guess(fptr),
  sol_der_x_value(0),sol_der_y_value(0),sol_value(0){
      this->set_equation_type(NON_LINEAR_EQUATION);
      this->tollerance=toll;
      if(fptr==null_f) this->set_initial_guess_projected();
    }
