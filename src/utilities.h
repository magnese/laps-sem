...
typedef enum { LINEAR_EQUATION,
    NON_LINEAR_EQUATION } eq_type;
...
void ghost_vec_disp(const vector<T>& vec, const int& c, M* mesh_ptr)
{
    ...
    for (j = vec.begin(); j != vec.end(); ++j) {
        if ((j - vec.begin()) % c == 0)
            cout << endl
                 << mesh_ptr->i((j - vec.begin()) / c + mesh_ptr->num_elements()) << ": ";
        cout << *j << " ";
    }
    cout << endl;
}
...
void node_vec_disp(const vector<T>& vec, const int& c, M* mesh_ptr)
{
    ...
    for (j = vec.begin(); j != vec.end(); ++j) {
        if ((j - vec.begin()) % c == 0)
            cout << " || " << mesh_ptr->get_nodes().i((j - vec.begin()) / c) << ": ";
        cout << *j << " ";
    }
    cout << endl;
}
...
{
    if (mesh_ptr->get_rank() == 0)
        cout << endl
             << "Breakpoint " << number << ":" << endl;
    for (int j = 0; j != mesh_ptr->get_size(); ++j) {
        if (mesh_ptr->get_rank() == j)
            cout << "status process " << j << " OK" << endl;
...
void parallel_value_disp(const T& value, M* mesh_ptr, const int& number = 0)
{
    ...
    for (int j = 0; j != mesh_ptr->get_size(); ++j) {
        if (mesh_ptr->get_rank() == j) {
            cout << "value = " << value << " on process " << j << endl;
        }
        MPI_Barrier(MPI_COMM_WORLD);
    }
    if (mesh_ptr->get_rank() == 0)
