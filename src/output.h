...
public:
  ...
  output(M* mesh_ptr,PS* ps_ptr):mesh(mesh_ptr),p_state(ps_ptr){};
...
void output<M,PS>::gmsh_dump(string file_name){
  ...
  if(mesh->get_rank()==0) mesh->write_file(file_name,WRITE_GHOSTS_OFF,p_state->get_nodal_values());
  else mesh->write_file(file_name,WRITE_GHOSTS_OFF);
