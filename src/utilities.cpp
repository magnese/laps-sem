...
double poly_val(const vector<double>& coeff,const double& x){
  double value(0);
  for(vector<double>::const_reverse_iterator j=coeff.rbegin();j!=coeff.rend();++j) value+=*j*pow(x,static_cast<int>(j-coeff.rbegin()));
  return value;
}

void poly_der(const vector<double>& coeff, vector<double>& der_coeff){
  der_coeff=coeff;
  for(vector<double>::reverse_iterator j=der_coeff.rbegin();j!=der_coeff.rend();++j) (*j)*=static_cast<int>(j-der_coeff.rbegin());
...
int min_positive(const int& x,const int& y,const int& z){
  if(x<0){
    ...
    if(y<0){
      ...
      if(z<0) return min(x,y);
      else return min(min(x,y),min(x,z));
    }
  }
}
