...
public:
  ...
  equation(M* mesh_ptr):mesh(mesh_ptr),basis(),integration(),map(0),temp_order(0),phi_values(0),der_phi_values(0),J_values(4),
    value(0),dirichlet_coeff(mesh->num_lines()),tollerance(0.0),num_iteration(0),projection_status(false),null_value(0){
      compute_basis();
...
void equation<M,B,G>::compute_basis(void){
  ...
  if(new_size>old_size){
    ...
    if(old_size>0)old_row_size=phi_values[0].size();
    ...
    if(mesh->get_bc_type(l)==DIRICHLET){
      ...
      for(int j=0;j!=(mesh->get_max_deg()+1);++j){
        for(int jj=0;jj!=(mesh->get_max_deg()+1);++jj){
          ...
          for(int k=0;k!=num_integration_nodes;++k) A[j*(mesh->get_max_deg()+1)+jj]+=phi(k,j)*phi(k,jj)*weight(k);
        }
      }
      ...
      dirichlet_coeff[l].resize(static_cast<unsigned>(mesh->get_max_deg()+1),0);
        for(int j=0;j!=(mesh->get_max_deg()+1);++j) dirichlet_coeff[l][j]=gsl_vector_get(x_gsl_ptr,j);
    }
  }
...
void equation<M,B,G>::disp_basis(const int& rank){
  if(mesh->get_rank()==rank){
    for(vector<vector<vector<double> > >::iterator iter=phi_values.begin();iter!=phi_values.end();++iter){
      cout<<endl<<"phi_"<<(iter-phi_values.begin())<<endl;
...
void equation<M,B,G>::disp_der_basis(const int& rank){
  if(mesh->get_rank()==rank){
    for(vector<vector<vector<double> > >::iterator iter=der_phi_values.begin();iter!=der_phi_values.end();++iter){
      cout<<endl<<"phi'_"<<(iter-der_phi_values.begin())<<endl;
      mat_disp<double>(*iter);
    }
  }
}
...
void equation<M,B,G>::disp_dir_coeff(const int& rank){
  if(mesh->get_rank()==rank){
    cout<<"Coefficients of the projections of Dirichlet boundary functions on the modal basis"<<endl<<endl;
    for(vector<vector<double> >::iterator iter=dirichlet_coeff.begin();iter!=dirichlet_coeff.end();++iter){
      ...
      for(vector<double>::iterator j=(*iter).begin();j!=(*iter).end();++j) cout<<*j<<" ";
      cout<<endl;
    }
  }
}
