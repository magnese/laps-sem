...
void mesh_geometry::write_msh(string file_name)
{
    ...
    if (file.is_open()) {
        ...
        for (int j = 0; j != my_nodes.num_nodes(); ++j)
            file << my_nodes.i(j) << " 15 3 0 " << my_nodes.i(j) << " 0 " << my_nodes.i(j) << endl;
        for (int j = 0; j != my_lines.num_lines(); ++j)
            file << my_lines.i(j) << " 1 3 0 " << my_lines.lt(j) << " 0 " << my_lines.p(j)[0] + 1 << " " << my_lines.p(j)[1] + 1 << endl;
...
void mesh_geometry::write_geo(string file_name)
{
    ...
    if (file.is_open()) {
        for (int i = 0; i != my_nodes.num_nodes(); ++i)
            file << "Point(" << my_nodes.i(i) << ") = {" << my_nodes.x(i) << ", " << my_nodes.y(i) << ", 0, 1e+22};" << endl;
        for (int j = 0; j != my_lines.num_lines(); ++j)
            file << "Line(" << my_lines.i(j) << ") = {" << my_lines.p(j)[0] + 1 << ", " << my_lines.p(j)[1] + 1 << "};" << endl;
    }
}
...
void mesh_geometry::split_mesh(string file_name)
{
    ...
    for (int k = 0; k != static_cast<int>(splitted_mesh.size()); ++k) {
        for (int j = 0; j != my_nodes.num_nodes(); ++j)
            (splitted_mesh[k].get_nodes()).add_node(my_nodes.nod(j));
    }
