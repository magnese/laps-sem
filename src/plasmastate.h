...
public:
  ...
  plasma_state(M* mesh_ptr,B* basis_ptr):mesh(mesh_ptr),basis(basis_ptr),map(0),nodal_values(mesh->num_nodes()),num_sample_points(0),
    non_nodal_values(0){}
...
void plasma_state<M,B>::disp_nodes_values(void){
  ...
  if(mesh->get_rank()==0){
    cout<<"Plasma state nodes values:"<<endl<<endl;
...
void plasma_state<M,B>::update(const int& num){
  ...
  for(int i=0;i!=mesh->num_nodes();++i) nodal_values[i]=map->u(map->vtx(i));
  ...
  for(int i=0;i!=(num_sample_points+2);++i) logical_coor[i]=-1+(2/(num_sample_points+1))*i;
  ...
  for(int j=0;j!=mesh->num_elements();++j){
    ...
    for(int k=0;k!=(num_sample_points+2);++k){
      for(int kk=0;kk!=(num_sample_points+2);++kk){
        ...
        if((k>0)&&(k<(num_sample_points+1))&&(kk>0)&&(kk<(num_sample_points+1))) flag=true;
        else{
          if((k>0)&&(k<(num_sample_points+1))&&(kk==(num_sample_points+1))){
            if(mesh->n(0,j)<0) flag=true;
            else{
              if(mesh->i(j)>mesh->i(mesh->n(0,j))) flag=true;
            }
          }
          else{
            if((k==(num_sample_points+1))&&(kk>0)&&(kk<(num_sample_points+1))){
              if(mesh->n(1,j)<0) flag=true;
              else{
                if(mesh->i(j)>mesh->i(mesh->n(1,j))) flag=true;
              }
            }
            else{
              if((k>0)&&(k<(num_sample_points+1))&&(kk==0)){
                if(mesh->n(2,j)<0) flag=true;
                else{
                  if(mesh->i(j)>mesh->i(mesh->n(2,j))) flag=true;
                }
              }
              else{
                if((k==0)&&(kk>0)&&(kk<(num_sample_points+1))){
                  if(mesh->n(3,j)<0) flag=true;
                  else{
                    if(mesh->i(j)>mesh->i(mesh->n(3,j))) flag=true;
                  }
                }
              }
            }
