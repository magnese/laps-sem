...
public:
  ...
  mesh_sem(const int& rank_value,const int& size_value,const int& n_refinements=0,string file_name="mesh.msh",flag_read flag1=READ_MONO,
    flag_write flag2=WRITE_OFF,const int& deg=2):mesh_parallel(rank_value,size_value,n_refinements,file_name,flag1,flag2),
    basis_deg(num_elements()),max_deg(0){
    if(deg<2){
      if(my_rank==0) cout<<"WARNING: Basis degree < 2, will be used degree = 2"<<endl<<endl;
      set_basis_deg(2);
    }
    else set_basis_deg(deg);
  }
  ...
  inline void set_basis_deg(const int& value){if(value>max_deg) max_deg=value;
    for(int i=0;i!=num_elements();++i) basis_deg[i]=value;}
  inline void set_basis_deg(const int& value,const int& pos){basis_deg[pos]=value;if(value>max_deg) max_deg=value;}
