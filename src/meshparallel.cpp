...
void mesh_parallel::set_ghosts_elm_proc(void)
{
    ...
    for (int j = 0; j != num_CPUs; ++j) {
        for (vector<long int>::iterator i = partition.begin(); i != partition.end(); ++i) {
            if (j == *i)
                elm_proc[j].push_back(i - partition.begin());
        }
    }
    for (int j = 0; j != num_CPUs; ++j) {
...
void mesh_parallel::pre_partitioning(void)
{
    ...
    if (my_rank == 0) {
        ...
        for (vector<long int>::iterator i = (elmdist.begin() + 1); i != elmdist.end(); ++i) {
            ...
            if (static_cast<int>((i - elmdist.begin())) < (num_exciding_elements + 1))
                ++temp;
            *i = temp;
        }
    }
    ...
    // lines broadcast
    if (my_rank == 0)
        temp_dim = my_lines.num_lines();
    MPI_Bcast(&temp_dim, 1, MPI_INT, 0, MPI_COMM_WORLD);
    if (my_rank != 0)
        my_lines.resize(temp_dim);
...
void mesh_parallel::unpack_par_elements_buffer(const int& num_new_elements)
{
    ...
    for (vector<int>::iterator j = receive_elements_buffer.begin(); j != receive_elements_buffer.end(); ++j) {
        ...
        if ((j - receive_elements_buffer.begin()) % 5 == 1)
            my_elements.set_v(*j, 0, old_size + (j - receive_elements_buffer.begin()) / 5);
        if ((j - receive_elements_buffer.begin()) % 5 == 2)
            my_elements.set_v(*j, 1, old_size + (j - receive_elements_buffer.begin()) / 5);
        if ((j - receive_elements_buffer.begin()) % 5 == 3)
            my_elements.set_v(*j, 2, old_size + (j - receive_elements_buffer.begin()) / 5);
        if ((j - receive_elements_buffer.begin()) % 5 == 4)
            my_elements.set_v(*j, 3, old_size + (j - receive_elements_buffer.begin()) / 5);
    }
}
...
void mesh_parallel::partitioning()
{
    ...
    for (vector<float>::iterator i = ubvec.begin(); i != ubvec.end(); ++i)
        *i = ubvecc;
    for (vector<float>::iterator i = tpwgts.begin(); i != tpwgts.end(); ++i)
        *i = 1 / (static_cast<float>(num_CPUs));
...
void mesh_parallel::compute_elmdist(void)
{
    ...
    if (my_rank == 0) {
        for (vector<long int>::iterator iter = (elmdist.begin() + 2); iter != elmdist.end(); ++iter)
            *iter += *(iter - 1);
    }
    MPI_Bcast(&(elmdist[0]), num_CPUs + 1, MPI_LONG, 0, MPI_COMM_WORLD);
}
