...
public:
  ...
  elements_list(void):my_elements(0),number_ghosts(0){}
  ...
  inline void add_ghost(element& new_element){my_elements.push_back(new_element);++number_ghosts;}
  inline void free_ghosts(void){my_elements.resize(static_cast<unsigned>(num_elements()));}
  inline void free_neighbors(void){for(vector<element>::iterator j=my_elements.begin();j!=my_elements.end();++j) {j->resize_neighbors(0,0);j->free_pos();}}
  inline void free(void){my_elements.resize(0);number_ghosts=0;}
