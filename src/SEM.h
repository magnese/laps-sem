...
public:
  ...
  SEM(M* mesh_ptr,E* equation_ptr,static_condesation_flag flag=STATIC_CONDENSATION_OFF):mesh(mesh_ptr),equation(equation_ptr),
    p_state(mesh,equation->get_basis()),map(mesh,equation->get_basis(),&p_state),static_condensation_status(false),values(0),
    num_max_iteration(50){
      if(flag==STATIC_CONDENSATION_ON) static_condensation_status=true;
      ...
      if(equation->get_equation_type()==LINEAR_EQUATION){
        if(static_condensation_status){
          if(mesh->get_rank()==0) cout<<"START: Assembling linear system with static condensation"<<endl;
          assemble();
          assemble_dirichlet_condition_LHS();
          assemble_dirichlet_condition_RHS();
          if(mesh->get_rank()==0) cout<<"END: Assembling linear system with static condensation"<<endl<<endl;
          if(mesh->get_rank()==0) cout<<"START: Solving linear system with static condensation"<<endl;
          solve();
          if(mesh->get_rank()==0) cout<<"END: Solving linear system with static condensation"<<endl<<endl;
        }
        else{
          if(mesh->get_rank()==0) cout<<"START: Assembling linear system without static condensation"<<endl;
          assemble_no_static_condensation();
          assemble_dirichlet_condition_LHS();
          assemble_dirichlet_condition_RHS();
          if(mesh->get_rank()==0) cout<<"END: Assembling linear system without static condensation"<<endl<<endl;
          if(mesh->get_rank()==0) cout<<"START: Solving linear system without static condensation"<<endl;
          solve_no_static_condensation();
          if(mesh->get_rank()==0) cout<<"END: Solving linear system without static condensation"<<endl<<endl;
        }
        scatter_solution();
      }
      else{
        if(static_condensation_status) nl_solve();
        else nl_solve_no_static_condensation();
      }
      if(mesh->get_rank()==0) cout<<"START: Storing solution in plasma state"<<endl;
      p_state.update((mesh->get_max_deg()-1));
      if(mesh->get_rank()==0) cout<<"END: Storing solution in plasma state"<<endl<<endl;
    }
...
void SEM<M,E,B>::create_petsc_variables(void){
  ...
  if(static_condensation_status){
    ...
    if(equation->get_equation_type()==NON_LINEAR_EQUATION) VecDuplicate(Xb,&Xb_sol);
  }
}
...
void SEM<M,E,B>::create_scatter(void){
  ...
  if(equation->get_equation_type()==NON_LINEAR_EQUATION) VecScatterCreateToAll(Xb_sol,&scatter_X,&X_seq);
  else VecScatterCreateToAll(Xb,&scatter_X,&X_seq);
...
void SEM<M,E,B>::free_petsc_variables(void){
  ...
  if(equation->get_equation_type()==NON_LINEAR_EQUATION) VecDestroy(&Xb_sol);
  ...
  if(static_condensation_status){
    ...
    if(equation->get_equation_type()==NON_LINEAR_EQUATION) VecDestroy(&Xi_sol);
  }
  ...
  for(int e=0;e!=mesh->num_elements();++e){
    for(int i=0;i!=(mesh->get_deg(e)+1);++i){
      for(int ii=0;ii!=(mesh->get_deg(e)+1);++ii){
        if(map.is_i(i,ii)) VecSetValue(Fi,map.inner(i,ii,e),equation->RHS(i,ii,e),ADD_VALUES);
        else{
          if(map.is_v(i,ii)){
            if((mesh->n(map.vtx_pos(i,ii),e)>-1)&&(mesh->n((map.vtx_pos(i,ii)+3)%4,e)>-1)) VecSetValue(Fb,map.vtx(i,ii,e),equation->RHS(i,ii,e),ADD_VALUES);
            else{
            ...
            else VecSetValue(Fb,map.edge(i,ii,e),pow(mesh->orientation(map.edge_pos(i,ii),e),max(i,ii))*equation->RHS(i,ii,e),ADD_VALUES);
          }
        }
        for(int j=0;j!=(mesh->get_deg(e)+1);++j){
          for(int jj=0;jj!=(mesh->get_deg(e)+1);++jj){
            if(map.is_i(j,jj)){
              if(map.is_i(i,ii)) MatSetValue(Aii,map.inner(i,ii,e),map.inner(j,jj,e),equation->LHS(i,ii,j,jj,e),ADD_VALUES);
              else{
                if(map.is_e(i,ii)){
...
void SEM<M,E,B>::assemble_no_static_condensation(void){
  ...
  for(int e=0;e!=mesh->num_elements();++e){
    for(int i=0;i!=(mesh->get_deg(e)+1);++i){
      for(int ii=0;ii!=(mesh->get_deg(e)+1);++ii){
        if(map.is_i(i,ii)) VecSetValue(Fb,map.inner(i,ii,e),equation->RHS(i,ii,e),ADD_VALUES);
        else{
          if(map.is_v(i,ii)){
            if((mesh->n(map.vtx_pos(i,ii),e)>-1)&&(mesh->n((map.vtx_pos(i,ii)+3)%4,e)>-1)) VecSetValue(Fb,map.vtx(i,ii,e),equation->RHS(i,ii,e),ADD_VALUES);
            else{
        ...
        for(int j=0;j!=(mesh->get_deg(e)+1);++j){
          for(int jj=0;jj!=(mesh->get_deg(e)+1);++jj){
            if(map.is_i(j,jj)){
              if(map.is_i(i,ii)) MatSetValue(Abb,map.inner(i,ii,e),map.inner(j,jj,e),equation->LHS(i,ii,j,jj,e),ADD_VALUES);
              else{
                if(map.is_e(i,ii)){
                  if(map.edge(i,ii,e)>(map.dim_ndof_nodes()+map.dim_ndof_edges()-1))
              ...
              if(map.is_e(j,jj)){
                if(map.is_i(i,ii)) MatSetValue(Abb,map.inner(i,ii,e),map.edge(j,jj,e),pow(mesh->orientation(map.edge_pos(i,ii),e),max(i,ii))*equation->LHS(i,ii,j,jj,e),ADD_VALUES);
                else{
                  if(map.is_e(i,ii)){
                    if(map.edge(i,ii,e)>(map.dim_ndof_nodes()+map.dim_ndof_edges()-1))
                      MatSetValue(Abb,map.edge(i,ii,e),map.edge(j,jj,e),pow(mesh->orientation(map.edge_pos(i,ii),e),max(i,ii))*pow(mesh->orientation(map.edge_pos(j,jj),e),max(j,jj))*equation->LHS(i,ii,j,jj,e),ADD_VALUES);
                ...
                if(map.is_i(i,ii)) MatSetValue(Abb,map.inner(i,ii,e),map.vtx(j,jj,e),equation->LHS(i,ii,j,jj,e),ADD_VALUES);
                else{
                  if(map.is_e(i,ii)){
                    if(map.edge(i,ii,e)>(map.dim_ndof_nodes()+map.dim_ndof_edges()-1))
                      MatSetValue(Abb,map.edge(i,ii,e),map.vtx(j,jj,e),pow(mesh->orientation(map.edge_pos(i,ii),e),max(i,ii))*equation->LHS(i,ii,j,jj,e),ADD_VALUES);
...
void SEM<M,E,B>::nl_solve_no_static_condensation(void){
  ...
  if(mesh->get_rank()==0) cout<<"START: Solving non linear system without static condensation"<<endl;
  ...
  while(flag&&equation->get_iter()<num_max_iteration){
    ...
    if(vec_norm<equation->get_toll()) flag=false;
    else{
      solve_no_static_condensation();
      VecAYPX(Xb_sol,1,Xb);
      scatter_solution();
    ...
    equation->inc_iter();
    if(mesh->get_rank()==0) cout<<"Iteration number "<<equation->get_iter()<<", L^2 norm of the residual "<<vec_norm<<endl;
  }

  if(equation->get_iter()==num_max_iteration){
    if(mesh->get_rank()==0) cout<<endl<<"ERROR: Reached maximum number of iterations before convergence"<<endl<<endl;
  }

  if(mesh->get_rank()==0) cout<<"END: Solving non linear system without static condensation"<<endl<<endl;
}
...
void SEM<M,E,B>::nl_solve(void){
  ...
  if(mesh->get_rank()==0) cout<<"START: Solving non linear system with static condensation"<<endl;
  ...
  while(flag&&equation->get_iter()<num_max_iteration){
    ...
    if((vec_norm_1+vec_norm_2)<equation->get_toll()) flag=false;
    else{
      solve();
      VecAYPX(Xb_sol,1,Xb);
      VecAYPX(Xi_sol,1,Xi);
    ...
    if(mesh->get_rank()==0) cout<<"Iteration number "<<equation->get_iter()<<", L^2 norm of the residual "<<vec_norm_1+vec_norm_2<<endl;
  }
  ...
  if(mesh->get_rank()==0) cout<<"END: Solving non linear system with static condensation"<<endl<<endl;
}
...
void SEM<M,E,B>::assemble_dirichlet_condition_LHS(void){
  ...
  if(mesh->get_rank()==0){
    for(int j=0;j!=(map.dim_ndof_nodes()+map.dim_ndof_edges());++j) MatSetValue(Abb,j,j,1,INSERT_VALUES);
  }
  MatAssemblyBegin(Abb,MAT_FINAL_ASSEMBLY);
  MatAssemblyEnd(Abb,MAT_FINAL_ASSEMBLY);
